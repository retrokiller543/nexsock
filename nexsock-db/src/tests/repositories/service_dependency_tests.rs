...
mod tests {
    ...
    async fn setup_services_for_test(repo: &ServiceRepository<'_>) -> (Service, Service) {
        ...
        repo.save(&mut service1).await.expect("Failed to save service1");
        ...
        repo.save(&mut service2).await.expect("Failed to save service2");
        (service1, service2)
    }
    ...
    async fn test_save_new_and_get_by_id() {
        ...
        assert_ne!(new_dependency.id, 0, "ServiceDependency ID should be populated after save");
    ...
    async fn test_save_update_existing() {
        ...
        dep_repo.save(&mut dependency).await.expect("Failed to save initial dependency");
        
        let original_dep_id = dependency.id;
        dependency.tunnel_enabled = true;
        dep_repo.save(&mut dependency).await.expect("Failed to update dependency");

        assert_eq!(dependency.id, original_dep_id, "Dependency ID should not change on update");
        ...
        assert!(fetched_dep.tunnel_enabled, "tunnel_enabled should be true after update");
    }
    ...
    async fn test_delete_by_id() {
        ...
        assert!(fetched_after_delete.is_none(), "Dependency should be None after deletion");
        ...
        assert!(delete_non_existent_result.is_err(), "Deleting a non-existent dependency should return an error");
    }

    #[tokio::test]
    async fn test_get_by_service_id() { // Corresponds to test_get_all_for_service
        let db = setup_in_memory_db().await.expect("Failed to setup in-memory DB");
        let service_repo = ServiceRepository::new(&db);
        let dep_repo = ServiceDependencyRepository::new(&db);
        ...
        service_repo.save(&mut s3).await.expect("Failed to save service3");

        // s1 -> s2
        let mut dep1 = ServiceDependency { id: 0, service_id: s1.id, dependent_service_id: s2.id, tunnel_enabled: false };
        dep_repo.save(&mut dep1).await.expect("Failed to save dep1");

        // s1 -> s3
        let mut dep2 = ServiceDependency { id: 0, service_id: s1.id, dependent_service_id: s3.id, tunnel_enabled: true };
        dep_repo.save(&mut dep2).await.expect("Failed to save dep2");

        // s2 -> s3 (should not be fetched when querying for s1)
        let mut dep3 = ServiceDependency { id: 0, service_id: s2.id, dependent_service_id: s3.id, tunnel_enabled: false };
        ...
        assert_eq!(dependencies_for_s1.len(), 2, "s1 should have 2 dependencies");
        assert!(dependencies_for_s1.iter().any(|d| d.dependent_service_id == s2.id && !d.tunnel_enabled));
        assert!(dependencies_for_s1.iter().any(|d| d.dependent_service_id == s3.id && d.tunnel_enabled));
        ...
        assert!(dependencies_for_s3.is_empty(), "s3 should have no dependencies");
    }
    ...
    async fn test_delete_many() {
        ...
        service_repo.save(&mut s3).await.expect("Failed to save service3 for delete_many");

        let mut dep1 = ServiceDependency { id: 0, service_id: s1.id, dependent_service_id: s2.id, tunnel_enabled: false };
        dep_repo.save(&mut dep1).await.expect("Failed to save dep1 for delete_many");
        
        let mut dep2 = ServiceDependency { id: 0, service_id: s1.id, dependent_service_id: s3.id, tunnel_enabled: false };
        dep_repo.save(&mut dep2).await.expect("Failed to save dep2 for delete_many");

        let mut dep_unaffected = ServiceDependency { id: 0, service_id: s2.id, dependent_service_id: s3.id, tunnel_enabled: false };
        dep_repo.save(&mut dep_unaffected).await.expect("Failed to save dep_unaffected for delete_many");

        let ids_to_delete = vec![dep1.id, dep2.id];
        dep_repo.delete_many(ids_to_delete).await.expect("Failed to delete_many dependencies");

        assert!(dep_repo.get_by_id(dep1.id).await.unwrap().is_none(), "dep1 should be deleted");
        assert!(dep_repo.get_by_id(dep2.id).await.unwrap().is_none(), "dep2 should be deleted");
        assert!(dep_repo.get_by_id(dep_unaffected.id).await.unwrap().is_some(), "dep_unaffected should still exist");
    }
    ...
    async fn test_get_dependencies_with_service_info() {
        ...
        service_repo.save(&mut s3).await.expect("Failed to save service3 for service_info test");

        // s1 -> s2 (tunnel_enabled = false)
        let mut dep1 = ServiceDependency { id: 0, service_id: s1.id, dependent_service_id: s2.id, tunnel_enabled: false };
        dep_repo.save(&mut dep1).await.expect("Failed to save dep1 for service_info test");

        // s1 -> s3 (tunnel_enabled = true)
        let mut dep2 = ServiceDependency { id: 0, service_id: s1.id, dependent_service_id: s3.id, tunnel_enabled: true };
        dep_repo.save(&mut dep2).await.expect("Failed to save dep2 for service_info test");
        ...
        assert_eq!(dep_infos_for_s1.len(), 2, "s1 should have 2 dependency infos");

        let info_s2 = dep_infos_for_s1.iter().find(|di| di.id == s2.id).expect("Dependency info for s2 not found");
        ...
        assert_eq!(info_s2.state, crate::models::service::ServiceStatus::Stopped.into());


        let info_s3 = dep_infos_for_s1.iter().find(|di| di.id == s3.id).expect("Dependency info for s3 not found");
        assert_eq!(info_s3.name, s3.name);
        assert!(info_s3.tunnel_enabled);
        assert_eq!(info_s3.state, crate::models::service::ServiceStatus::Stopped.into());
        ...
        assert!(dep_infos_for_s2.is_empty(), "s2 should have no dependency infos");
    }
    ...
    async fn test_get_dependencies_response() {
        ...
        // s1 -> s2
        let mut dep1 = ServiceDependency { id: 0, service_id: s1.id, dependent_service_id: s2.id, tunnel_enabled: false };
        dep_repo.save(&mut dep1).await.expect("Failed to save dep1 for response test");
        ...
        assert_eq!(response_for_s1.dependencies.len(), 1, "s1 should have 1 dependency in response");
        ...
        assert!(response_for_s2.dependencies.is_empty(), "s2 should have no dependencies in response");
    }
}
