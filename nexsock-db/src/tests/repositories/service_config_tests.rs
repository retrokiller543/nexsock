...
mod tests {
    ...
    async fn test_save_new_and_get_by_id() {
        ...
        assert_ne!(
            new_config.id, 0,
            "ServiceConfig ID should be populated after save"
        );
    ...
    async fn test_save_update_existing() {
        ...
        let mut config = ServiceConfig::new("original.env".to_string(), ConfigFormat::Env, None);
        repo.save(&mut config)
            .await
        ...
        assert_eq!(
            ...
            "Config ID should not change on update"
        );
    ...
    async fn test_delete_by_id() {
        ...
        let mut config_to_delete =
            ServiceConfig::new("delete_me.env".to_string(), ConfigFormat::Env, None);
        ...
        assert!(
            ...
            "Config should be None after deletion"
        );
        ...
        assert!(
            ...
            "Deleting a non-existent config should return an error"
        );
    }
}
