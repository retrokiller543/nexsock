...
impl DependencyManagement for DependencyManager2 {
    async fn add_dependency(&self, payload: &AddDependencyPayload) -> crate::error::Result<()> {
        ...
        let parent_service_id = self.service_repository.extract_valid_id_from_ref(service).await?;
        let dependent_service_id = self.service_repository.extract_valid_id_from_ref(dependent_service).await?;
    ...
    async fn remove_dependency(&self, payload: &RemoveDependencyPayload) -> crate::error::Result<()> {
        ...
        let dependencies = self.dependency_repository.get_by_service_id(parent_service_id).await?;

        for dependency in dependencies {
            if dependency.dependent_service_id == dependent_service_id {
                self.dependency_repository.delete_by_id(dependency.id).await?;
                return Ok(());
            }
        }
    ...
    async fn list_dependencies(&self, payload: &ServiceRef) -> crate::error::Result<ListDependenciesResponse> {
        let service = self.service_repository.get_by_service_ref(payload).await?
            .ok_or_else(|| anyhow!("No service with this name or id"))?;

        let service_id = service.id;
        let name = service.name;

        self.dependency_repository.get_dependencies_response(service_id, name).await.map_err(Into::into)
    }
}